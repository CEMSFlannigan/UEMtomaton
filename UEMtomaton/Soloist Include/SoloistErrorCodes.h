/// \file SoloistErrorCodes.h
/// \brief Contains the error code enumeration.
/// 
/// These are the different errors that you can get by calling SoloistGetLastError()
/// 
/// Copyright (c) Aerotech, Inc. 2010-2013.
/// 

#ifndef __Soloist_ERROR_CODES_H__
#define __Soloist_ERROR_CODES_H__

/// \brief The enumeration containing the error codes that can be returned from the Soloist functions.
typedef enum {
	/// \brief No error
	ErrorCode_NoError = 0,
	...
	/// \brief The format of the plugin image file is invalid : [SpecificMessage]
	ErrorCode_InvalidCustomerPluginImageFileFormat = 36,
	...
	/// \brief Command [CommandName] is an invalid command
	ErrorCode_InvalidCommand = 117,
	/// \brief Error loading the customer plugin image file : [SpecificMessage]
	ErrorCode_CustomerPluginImageLoadError = 118,
	/// \brief Error configuring the customer debug image file : [SpecificMessage]
	ErrorCode_CustomerDebugImageConfigError = 119,
	/// \brief Error retrieving data from the customer debug image file : [SpecificMessage]
	ErrorCode_CustomerDebugImageDataRetrieveError = 120,
	...
	/// \brief The header received from [ControllerID] did not match: received [ReceivedHeader], expected [ExpectedHeader]
	ErrorCode_ReceiveHeaderMismatch = 162,
	/// \brief A timeout occurred while waiting for a command response from [ControllerID]
	ErrorCode_ReceiveTimeout = 163,
	/// \brief An attempt to communicate to device [ControllerID] returned the device error: [LastError]
	ErrorCode_PCDeviceFailure = 164,
	/// \brief A timeout occurred while sending a command to [ControllerID]
...
#endif // __Soloist_ERROR_CODES_H__
