...
void WriteULG::WriteData(std::string FileName, std::string val1, std::string val2, std::string val3, std::string val4, std::string val5, std::string val6, std::string val7, std::string val8, int row)
{
	...
	if (row == 1)
	{
		...
		OpenFile << "[Number of Steps\t]\t" + val3 + "\n" + "[Number of Scans\t]\t" + val6 + "\n";
		OpenFile << "\t\n";
		OpenFile << "[Column Descriptions\t]:\tStep,Delay,Scan,Scan Step,File Name,Time\n";
		OpenFile << "\n";
		OpenFile << "\t\n";
		OpenFile << ",,,,,\n";
		OpenFile << val4 + "," + val5 + "," + val6 + "," + val7 + "," + val8 + "," + msclr::interop::marshal_as<std::string>(gcnew System::String(ctime(&mytime))) + "\n";
